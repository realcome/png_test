...
def reorder_imports(input_dir, output_dir, architecture, chrome_name):
  ...
  exe_name = chrome_name + '.exe'
  input_image = os.path.join(input_dir, exe_name)
  output_image = os.path.join(output_dir, exe_name)

  """Disable 360se.exe dep protect,by zongxiaobin
  """
  editbin_exe = os.path.join(
    __file__,
    '..\\..\\..\\tools\\360\\build_vstool\\editbin.exe')

  nxcompatargs = [editbin_exe,'/nxcompat:no','%s' % input_image ]
  subprocess.call(nxcompatargs)
...
#  assert found_elf, 'chrome_elf.dll import not found'
  ...
  exe_name_dot = exe_name + '.*'
  for fname in glob.iglob(os.path.join(input_dir, exe_name_dot)):
    shutil.copy(fname, os.path.join(output_dir, os.path.basename(fname)))
  return 0


def main(argv):
  usage = 'reorder_imports.py -i <input_dir> -o <output_dir> -a <target_arch> -n <chrome_name>'
  ...
  parser.add_option('-a', '--arch', help='architecture of build (optional)',
      default='ia32')
  parser.add_option('-n', '--name', help='architecture of build (optional)',
      default='ia32')
