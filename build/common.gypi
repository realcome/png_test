...
# since gyp_chromium is automatically forcing its inclusion.
{
  ...
  'variables': {
    ...
    'variables': {
      ...
      'for_360se%': 0,
      'enable_adfilter%': 1,
      'enable_guess_ulike%': 0,
      'enable_kantu%': 0,
      'enable_google_sync%': 0,
      'enable_component_updater%': 0,
      'enable_delay_gpu_compositor%': 0,

      # Control whether use 360 virtual keyboard, for secure input like passwords.
      'use_360_vkb%': 0,
    ...
    'enable_dom_distiller%': '<(enable_dom_distiller)',

    # Turns on compiler optimizations in V8 in Debug build.
    'v8_optimized_debug%': 1,

    'use_360hack%': '<(use_360hack)',
    'for_360chrome%': '<(for_360chrome)',
    'for_360se%': '<(for_360se)',
    'enable_adfilter%': '<(enable_adfilter)',
    # This control guess-ulike feature.
    'enable_guess_ulike%': '<(enable_guess_ulike)',
    'enable_kantu%': '<(enable_kantu)',
    'use_360_vkb%': '<(use_360_vkb)',
    'enable_google_sync%': '<(enable_google_sync)',
    'enable_delay_gpu_compositor%': '<(enable_delay_gpu_compositor)',
    ...
    'conditions': [
      ...
      ['use_360hack==1 and for_360se==1', {
        'use_360_vkb%': 1,
        'grit_defines': ['-D', 'use_360_vkb'],
        'enable_kantu%': 1,
        'enable_guess_ulike%': 1,
        'enable_autofill_dialog%': 0,
      }],
      ['use_360hack==1 and for_360chrome==1', {
        'enable_autofill_dialog%': 1,
      }],
      ...
      # use_360hack, grit prefers lower case.
      ['enable_guess_ulike==1', {
        'grit_defines': ['-D', 'enable_guess_ulike'],
      }],
      ['use_360hack==1', {
        'grit_defines': ['-D', 'use_360hack'],
      }],
      ['for_360chrome==1', {
        'grit_defines': ['-D', 'for_360chrome'],
      }],
      ['for_360se==1', {
        'grit_defines': ['-D', 'for_360se'],
      }],
      ['enable_adfilter==1', {
        'grit_defines': ['-D', 'enable_adfilter'],
      }],
      ['enable_kantu==1', {
        'grit_defines': ['-D', 'enable_kantu'],
      }],
      ['chromeos==1', {
      ...
      ['enable_google_sync==1', {
        'grit_defines': ['-D', 'enable_google_sync'],
      }],
      ['enable_delay_gpu_compositor==1', {
        'grit_defines': ['-D', 'enable_delay_gpu_compositor'],
      }],
      ['enable_component_updater==1', {
        'grit_defines': ['-D', 'enable_component_updater'],
      }],
      ['enable_dom_distiller==1', {
        'grit_defines': ['-D', 'enable_dom_distiller'],
      }],
      ['enable_plugins!=0', {
  ...
  'target_defaults': {
    ...
    'conditions': [
      ...
      ['for_360chrome==1', {
        'defines': ['FOR_360CHROME'],
      }],
      ['for_360se==1', {
        'defines': ['FOR_360SE'],
      }],
      ['enable_guess_ulike==1', {
        'defines': ['ENABLE_GUESS_ULIKE'],
      }],
      ['enable_adfilter==1', {
        'defines': ['ENABLE_ADFILTER'],
      }],
      ['use_360_vkb==1', {
        'defines': ['USE_360_VKB'],
      }],
      ['enable_kantu==1', {
        'defines': ['ENABLE_KANTU'],
      }],
      ...
      ['fieldtrial_testing_like_official_build==0 and branding!="Chrome" and branding!="Chromium"', {
